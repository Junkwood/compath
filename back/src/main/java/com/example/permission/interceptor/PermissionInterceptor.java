...
public class PermissionInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (projectIdStr == null) {
            // 이렇게 로그를 남겨두면 나중에 뚫렸을 때 원인 찾기가 엄청 쉽습니다!
        ...
        String targetUrl;
        //원본 패턴이 permissions 테이블에 명시적으로 등록되어있는지 확인.
        ...
        if(targetUrl == null){
            log.debug("매핑 안됨");
            return true; //매핑 안된건 일단 통과시키는 데 작동하는 거 보고 false로 고칠 수도 있음.
        }
        //최종 권한 검사
        Integer hasPermission = permissionMapper.checkUserPermission(userId,projectId,method,targetUrl);
        if(hasPermission>0){
            log.warn("권한 성공 : userId={}, projectId={}, Method={}, TargetUrl={}", userId, projectId, method, targetUrl);
            return true;//성공
        }else{
            log.warn("권한 거부 : userId={}, projectId={}, Method={}, TargetUrl={}", userId, projectId, method, targetUrl);
    ...
    private String mapPackageToDbUrl(String packageName) {
        ...
        else if (packageName.contains(".meeting")) return "/api/meetings";
        return null;
    }
