<template>
  <main class="bg-white dark:bg-slate-900">
    <div class="relative md:flex">
      ...
      <!-- 왼쪽 배경 패널 -->
      <div class="hidden md:flex md:w-1/2 bg-slate-800 flex-col justify-between p-10">
        <!-- 로고 -->
        <div class="flex items-center gap-2">
          ...
          <span class="text-white font-semibold text-lg tracking-tight">ComPath</span>
        </div>
      ...
      <div class="md:w-1/2 min-h-screen flex items-center justify-center p-6">
        <div class="w-full max-w-sm">
          ...
          <div class="flex items-center gap-2 mb-8 md:hidden">
            ...
            <span class="text-slate-800 dark:text-white font-semibold text-lg">Plana</span>
          </div>

          <h2 class="text-2xl font-bold text-slate-800 dark:text-white mb-1">어서오세요 👋</h2>
          <p class="text-sm text-slate-500 dark:text-slate-400 mb-8">
            계정에 로그인하여 프로젝트를 관리하세요
          </p>

          <!-- 오류 메시지 -->
          <div v-if="errorMsg" class="mb-5 bg-rose-50 dark:bg-rose-900/20 border border-rose-200 dark:border-rose-700 rounded-lg px-4 py-3 flex items-center gap-2">
            <svg class="w-4 h-4 text-rose-500 shrink-0" fill="currentColor" viewBox="0 0 20 20">
              <path fill-rule="evenodd"
                    d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z"
                    clip-rule="evenodd" />
            </svg>
            <p class="text-sm text-rose-600 dark:text-rose-400">{{ errorMsg }}</p>
          </div>
          ...
          <form @submit.prevent="handleSubmit" class="space-y-4">
            ...
            <!-- 사번 -->
            <div>
              <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5" for="id">
                사번
              </label>
              <input
                  id="id"
              ...
              />
              <p v-if="fieldError.id" class="mt-1 text-xs text-rose-500">{{ fieldError.id }}</p>
            </div>
            ...
            <div>
              <div class="flex items-center justify-between mb-1.5">
                ...
                <a href="#" class="text-xs text-indigo-500 hover:text-indigo-600 font-medium">
                  비밀번호 재설정
                </a>
              </div>
              <div class="relative">
              ...
              <p v-if="fieldError.password" class="mt-1 text-xs text-rose-500">{{ fieldError.password }}</p>
            </div>
